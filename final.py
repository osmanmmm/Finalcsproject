def run(): # Entire program runs in one singular function.
    ...
    for i in range(500): # This for loop draws 500 "stars"
        x = randint(0, 1000)   # at random points within the window.
        y = randint(0, 1000)
    ...
    cometflown = False      # This check is set so if the comet has already been flown (and therefore been undrawn),   # the program will not attempt to continue "flying" the comet.



    menu = Text(Point(375, 10), """Type "q" to quit the simulation.""") # This is the text for the quit instructions on the top middle of the screen.
    ...
    while not window.isClosed():  # checks that the window is not closed: 
        if not cometflown: # chekcs if the comet has not flown 
            pt=window.checkMouse()  # gets a point from the mouse 
            if pt!=None:     # checks if the piont is something 
                cometUserpts = window.getMouse()  # gets the user points from the window 
                cometUserXpt = cometUserpts.getX()
                cometUserYpt = cometUserpts.getY()

                distanceComet2Pt = sqrt(((cometinitialX - cometUserXpt) ** 2) + ((cometinitialY - cometUserYpt) ** 2)) # calculates the distance between the user points and the commet points 
                Xvector = ((cometUserXpt - cometinitialX) / distanceComet2Pt) *20
                Yvector = ((cometUserYpt - cometinitialY) / distanceComet2Pt) *20
                cometflown=True 
        key=window.checkKey()    
        if key=="q":
            quit()
            break 
        if cometflown:  # checks and moves the comet to an x direction and y direction 
            ...
            Earth_CometDistance = sqrt(((int(cometXpoint - earth.x))**2) + ((int(cometYpoint - earth.y))**2))
                # Will continually compare the distance between the comet and the planet to the sum of the planet's
            ...
            if Mercury_CometDistance < (mercury_radius + cometRadius):  # checks a collision for mercury with the comet due to distances 
            ...
            if Venus_CometDistance < (venus_radius + cometRadius):  #Checks a collision for comet with  venus due to distances 
            ...
            if Earth_CometDistance < (earth_radius + cometRadius):  # checks a collision for the comet with mercury due to distances 
